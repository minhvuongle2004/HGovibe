...
class AccountScreen extends StatelessWidget {
  ...
  Widget _buildVerifyEmailCard(BuildContext context, UserProvider provider) {
    return Card(
  ...
  Widget _buildAccountActions(BuildContext context, UserProvider provider) {
    final user = provider.user!;
  ...
  String _formatProviderSummary(List<String> providerIds) {
    ...
    final displayNames = providerIds.map(_providerDisplayName).toSet().toList();
        ...
        children: [
          ...
          ...providers.map(
            (p) => ListTile(
              ...
              title: Text(_providerDisplayName(p.providerId)),
  ...
  Future<void> _handleProviderAction(
    ...
    try {
      ...
      if (context.mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(e.message ?? 'Lỗi: ${e.code}')));
      }
    } catch (error) {
      if (context.mounted) {
        Navigator.pop(context);
