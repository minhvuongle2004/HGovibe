...
class AIPlaceRecommendationService {
  ...
  Future<String?> _callGemini(String prompt) async {
    ...
    for (int i = 0; i < futures.length; i++) {
      ...
      futures[i].then((result) {
        ...
        completedCount++;
        debugPrint('📊 Model $modelName hoàn thành sau ${elapsed}s (result: ${result != null && result.isNotEmpty ? "có dữ liệu" : "null/empty"})');
        
        if (!hasSuccess && result != null && result.isNotEmpty) {
          hasSuccess = true;
          if (!completer.isCompleted) {
            completer.complete(result);
            debugPrint('✅ Lấy kết quả từ model $modelName (model thứ ${i + 1}, sau ${elapsed}s)');
          }
        } else if (completedCount == futures.length && !completer.isCompleted) {
          // Tất cả đã hoàn thành nhưng không có kết quả nào
          debugPrint('⚠️ Tất cả ${futures.length} models đã hoàn thành nhưng không có kết quả hợp lệ');
  ...
  Future<String?> _callSingleModel(String model, String prompt) async {
    final startTime = DateTime.now();
    debugPrint('🔄 Đang gọi model $model... (timeout: ${_requestTimeout.inSeconds}s)');
    try {
      final url = Uri.parse(
        ...
        if (candidates != null && candidates.isNotEmpty) {
          ...
          if (finishReason != null && finishReason != 'STOP') {
            if (finishReason == 'MAX_TOKENS') {
              debugPrint('⚠️ Model $model finishReason=MAX_TOKENS (đã dùng hết token, có thể vẫn có partial output)');
            } else {
          ...
          if (parts != null && parts.isNotEmpty) {
            ...
            if (text != null && text.trim().isNotEmpty) {
              debugPrint('✅ Gemini trả kết quả với model $model sau ${elapsed}s (${text.length} chars)');
              return text;
            } else {
              debugPrint('⚠️ Model $model trả về nhưng text rỗng sau ${elapsed}s');
              debugPrint('📄 Response preview: ${response.body.substring(0, response.body.length > 500 ? 500 : response.body.length)}');
            }
          } else {
            debugPrint('⚠️ Model $model trả về nhưng không có parts sau ${elapsed}s');
            debugPrint('📄 Response structure: candidates=${candidates.length}, content=${content != null ? "exists" : "null"}, parts=${parts != null ? "exists but empty" : "null"}');
            debugPrint('📄 Response preview: ${response.body.substring(0, response.body.length > 500 ? 500 : response.body.length)}');
          }
        } else {
          debugPrint('⚠️ Model $model trả về nhưng không có candidates sau ${elapsed}s');
      ...
      final elapsed = DateTime.now().difference(startTime).inSeconds;
      debugPrint('⏱️ Timeout khi gọi model $model sau ${elapsed}s (timeout limit: ${_requestTimeout.inSeconds}s)');
      return null;
    } catch (e, stack) {
      final elapsed = DateTime.now().difference(startTime).inSeconds;
