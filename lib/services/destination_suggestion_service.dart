...
class DestinationSuggestionService {
  ...
  static final DestinationSuggestionService instance = DestinationSuggestionService._();
  ...
  Future<List<SuggestedDestination>> suggestNearbyDestinations(
    ...
    final cacheKey = _getCacheKey(targetDestination, maxDistance, limit, excludeIds);
    ...
    try {
      ...
      if (allDestinations != null) {
        ...
        if (destinationsToSearch.isEmpty) {
          debugPrint('⚠️ No destinations found in same city, trying all destinations');
          destinationsToSearch = await DestinationService.getAllDestinations(limit: 200);
        }
      }
      ...
      final filteredDestinations = destinationsToSearch.where((dest) {
        ...
        if (excludeIds != null && dest.id != null && excludeIds.contains(dest.id)) {
          return false;
        }
        
        // Chỉ lấy destinations trong cùng thành phố hoặc tỉnh
        final sameCity = dest.location.city.toLowerCase() == 
            targetDestination.location.city.toLowerCase();
        final sameProvince = dest.location.district.isNotEmpty && 
      ...
      debugPrint('🔍 Found ${filteredDestinations.length} destinations in same region');
      ...
      for (var dest in filteredDestinations) {
        try {
          ...
          if (result != null && result.distance > 0 && result.distance <= maxDistance) {
            suggestions.add(SuggestedDestination(
              destination: dest,
