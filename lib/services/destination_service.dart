...
class DestinationService {
  ...
  static final CollectionReference _destinationsRef =
      _db.collection(AppConstants.destinationsCollection);
  ...
  static Future<List<Destination>> getAllDestinations({
    ...
    try {
      ...
      // Sắp xếp thủ công
      uniqueResults.sort((a, b) => b.popularityScore.compareTo(a.popularityScore));
    ...
    try {
      ...
      for (var doc in allSnapshot.docs) {
        try {
          ...
          final matchDescription = dest.description.toLowerCase().contains(queryLower);
          final matchTags = dest.tags.any((tag) => tag.toLowerCase().contains(queryLower));
          final matchCity = dest.location.city.toLowerCase().contains(queryLower);
      ...
      print('✅ Tìm thấy ${uniqueResults.length} kết quả (${results.length - uniqueResults.length} duplicates đã loại bỏ) cho "$query"');
  ...
  static Future<List<Destination>> getDestinationsByCity(
    ...
    try {
      ...
      print('✅ Load được ${finalResults.length} destinations cho $city (${results.length - uniqueResults.length} duplicates đã loại bỏ)');
      return finalResults;
    } catch (e) {
  ...
  static Future<List<Destination>> getRecommendedDestinations({
    ...
    try {
      ...
      print('📊 Tổng số documents trong Firestore: ${totalSnapshot.docs.length}');
      ...
      final activeSnapshot = await _destinationsRef
          ...
          .limit(1000)
          .get();
      print('📊 Số documents với status = active: ${activeSnapshot.docs.length}');
      ...
      print('📊 Query lấy ${snapshot.docs.length} documents (limit: $maxLimit)');
      ...
      if (results.length != uniqueResults.length) {
        print('⚠️ Phát hiện ${results.length - uniqueResults.length} duplicates trong kết quả query');
      }
      ...
      final finalResults = uniqueResults.take(limit).toList();
      print('✅ Load được ${finalResults.length} recommended destinations (${results.length - uniqueResults.length} duplicates đã loại bỏ)');
      return finalResults;
    } catch (e) {
      print('❌ Lỗi getRecommendedDestinations: $e');
      print('💡 Có thể cần tạo index trong Firestore hoặc data chưa được import');
  ...
  static Future<List<Destination>> getDestinationsByTags(
    ...
    try {
      ...
      final results = snapshot.docs
          ...
          .whereType<Destination>()
          .where((dest) =>
              tags.any((tag) => dest.tags.any((t) => t.toLowerCase() == tag.toLowerCase())))
          .toList();
