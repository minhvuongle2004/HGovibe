...
import 'package:admin_web/models/tours/tour_package.dart';
import 'package:admin_web/models/tours/participant_info.dart';
import 'package:admin_web/models/tours/contact_info.dart';
...
class TourBooking {
  ...
  final String? paymentRequestId; // ID request gửi tới gateway
  final Map<String, dynamic>? paymentGatewayRawData; // Raw payload từ gateway
  ...
  final String? notes; // Ghi chú từ admin (legacy single note)
  final List<BookingAdminNote> adminNotes; // Danh sách ghi chú admin
  ...
  factory TourBooking.fromMap(Map<String, dynamic> map, {String? id}) {
    ...
    final adminNotes = (map['adminNotes'] as List<dynamic>?)
            ?.map((e) => BookingAdminNote.fromMap(e as Map<String, dynamic>))
            .toList() ??
        [];
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      if (adminNotes.isNotEmpty)
        'adminNotes': adminNotes.map((note) => note.toMap()).toList(),
    };
  }
...
class BookingAdminNote {
  ...
  factory BookingAdminNote.fromMap(Map<String, dynamic> map) {
    final createdAt = map['createdAt'];
    DateTime noteDate;
    if (createdAt is Timestamp) {
      noteDate = createdAt.toDate();
    } else if (createdAt is String) {
      noteDate = DateTime.tryParse(createdAt) ?? DateTime.now();
    } else {
      noteDate = DateTime.now();
    }

    return BookingAdminNote(
      content: map['content'] ?? '',
      adminId: map['adminId'],
