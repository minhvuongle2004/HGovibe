...
class TripService {
    ...
    try {
      ...
      for (var entry in updates.entries) {
        if (entry.key == 'plannedDate' && entry.value is DateTime) {
          firestoreUpdates['plannedDate'] = Timestamp.fromDate(
  ...
  Stream<List<TripItem>> watchTripItems(String userId, String tripId) {
    final itemsRef = _itemsRefForTrip(userId, tripId);
    return itemsRef.orderBy('order', descending: false).snapshots().asyncMap((
      snapshot,
    ) async {
      final items = <TripItem>[];
      for (var doc in snapshot.docs) {
  ...
  Future<void> _reorderItemsAfterDelete(String userId, String tripId) async {
    try {
      final items = await getTripItems(userId, tripId);
      final batch = _firestore.batch();
      final itemsRef = _itemsRefForTrip(userId, tripId);
  ...
  Future<void> saveAISuggestions(
    ...
    try {
      ...
      if (suggestions.isNotEmpty) {
        ...
        await batch.commit();
        debugPrint(
          '✅ Đã lưu ${suggestions.length} AI suggestions vào Firestore cho trip $tripId',
        );
      }
    } catch (e, stack) {
  ...
  Future<List<AIActivitySuggestion>> getAISuggestions(
    ...
    try {
      ...
      debugPrint(
        '✅ Loaded ${suggestions.length} AI suggestions từ Firestore cho trip $tripId',
      );
      return suggestions;
    } catch (e, stack) {
  ...
  Future<void> deleteAISuggestions(String userId, String tripId) async {
    try {
      final suggestionsRef = _aiSuggestionsRefForTrip(userId, tripId);
      final snapshot = await suggestionsRef.get();
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        await batch.commit();
        debugPrint(
          '✅ Đã xóa ${snapshot.docs.length} AI suggestions cho trip $tripId',
        );
      }
    } catch (e, stack) {
  ...
  Future<void> updateAISuggestionStatus(
    ...
    try {
      final suggestionRef = _aiSuggestionsRefForTrip(
        userId,
        tripId,
      ).doc(suggestionId);
