...
import 'package:smart_travel_app/config/auth/social_login_config.dart';
...
import 'exceptions/auth_exceptions.dart'
    show AuthCancelledException, AccountExistsWithDifferentCredentialException;

class AuthService {
  AuthService._({FirebaseAuth? auth, UserProfileService? profileService})
  ...
  Future<AppUser> signUpWithEmail({
    ...
    try {
      ...
      return AppUser.fromFirebaseUser(firebaseUser, profile: profile);
  ...
  Future<AppUser> _linkWithCredential(
    ...
    return AppUser.fromFirebaseUser(result.user!, profile: profile);
  }
  ...
  _buildFacebookCredential() async {
    ...
    try {
      fbData = await FacebookAuth.instance.getUserData(fields: "name,email");
    } catch (_) {
      // ignore
    }
