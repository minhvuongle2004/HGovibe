...
class AIDestinationService {
  ...
  Future<Destination> createDestinationFromSuggestion(
    ...
    AIDestinationValidator.logValidationResult(
      validation,
    ...
    if (validation.warnings.isNotEmpty) {
      debugPrint(
        '⚠️ Validation warnings (non-blocking): ${validation.warnings.join('; ')}',
      );
    }
    ...
    if (suggestion.latitude != null && suggestion.longitude != null) {
      ...
      if (existingDestinations.isNotEmpty) {
        final duplicateCheck =
            await AIDestinationValidator.checkDuplicateLocation(
              suggestion.latitude!,
              suggestion.longitude!,
        ...
        if (!duplicateCheck.isValid) {
          debugPrint(
            '⚠️ Duplicate location detected: ${duplicateCheck.errors.join('; ')}',
          );
    ...
    try {
      ...
      for (final destData in nearbyDestinations) {
        ...
        if (_calculateNameSimilarity(existingName, suggestionName) > 0.8) {
          debugPrint(
            '🔍 Found similar destination: $existingName vs $suggestionName',
          );
          return Destination.fromFirestore(
            _firestore.doc('destinations/${destData['id']}')
  ...
  Future<List<Map<String, dynamic>>> _getNearbyDestinations(
    ...
    try {
      ...
      debugPrint(
        '🔍 Found ${results.length} destinations within ${radiusKm}km',
      );
      return results;
    } catch (e, stack) {
