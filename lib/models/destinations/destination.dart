...
class Destination {
  ...
  String? get formattedPrice {
    if (estimatedCost != null && estimatedCost!.totalSuggested > 0) {
      return 'Từ ₫ ${estimatedCost!.totalSuggested.toString().replaceAllMapped(RegExp(r'(\d)(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}';
    } else if (entranceFee != null && entranceFee! > 0) {
      return 'Từ ₫ ${entranceFee.toString().replaceAllMapped(RegExp(r'(\d)(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}';
    }
    return null;
  }
...
class DayHours {
  ...
  factory DayHours.fromMap(Map<String, dynamic> map) {
    return DayHours(open: map['open'] ?? '', close: map['close'] ?? '');
  }
...
class SeasonalEvent {
  ...
  factory SeasonalEvent.fromMap(Map<String, dynamic> map) {
    return SeasonalEvent(
      name: map['name'] ?? '',
      months: Destination._toIntList(map['months']),
      description: map['description'] ?? '',
    );
  }

  Map<String, dynamic> toMap() {
    return {'name': name, 'months': months, 'description': description};
  }
}
...
class Parking {
  ...
  Map<String, dynamic> toMap() {
    return {'available': available, 'fee': fee, 'note': note};
  }
}

/// Model cho thông tin tiếp cận
class Accessibility {
