...
class AIPlanViewer extends StatefulWidget {
  ...
  const AIPlanViewer({super.key, required this.tripId, required this.userId});
...
class _AIPlanViewerState extends State<AIPlanViewer> {
  ...
  void _loadPlan() async {
    ...
    try {
      final plan = await AIPlanService.instance.getCurrentAIPlan(
        widget.userId,
        widget.tripId,
      );
      if (mounted) {
  ...
  void _subscribeToPlan() {
    AIPlanService.instance
        .watchCurrentAIPlan(widget.userId, widget.tripId)
        .listen((plan) {
          if (mounted) {
            setState(() {
              _currentPlan = plan;
