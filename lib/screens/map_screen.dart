...
class _MapScreenContentState extends State<_MapScreenContent> {
  ...
  void _onCameraIdle() {
    ...
    if ((newZoom - _currentZoom).abs() > 0.25) {
      ...
      final provider = context.read<DestinationProvider>();
      _syncDestinationMarkers(
        provider.recommendedDestinations,
        force: true,
      );
    } else {
      _currentZoom = newZoom;
  ...
  Future<void> _syncDestinationMarkers(
    ...
    if (_selectedDestination != null &&
        !effectiveList.any(
          (dest) => _destinationKey(dest) == _destinationKey(_selectedDestination!),
        )) {
  ...
  List<_MarkerEntry> _buildMarkerEntries(List<Destination> destinations) {
    ...
    for (final destination in effective) {
      ...
      clusters.putIfAbsent(bucketKey, () => _ClusterAccumulator()).add(destination);
    }
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  leading: CircleAvatar(
                    radius: 16,
                    backgroundColor:
                        _colorWithOpacity(_activeProfileOption.color, 0.15),
                    child: Icon(
                      Icons.navigation,
                      size: 16,
  ...
  Color _profileColor(String profileId) {
    return _RoutingProfileOption.values.firstWhere(
  ...
  Widget build(BuildContext context) {
    ...
    final offlineIndicatorTop =
        _isRoutingActive ? media.padding.top + 12 : media.padding.top + 90;
        ...
        children: [
          ...
          if (_isOffline)
            Positioned(
              ...
              left: 16,
              child: Chip(
                avatar: const Icon(Icons.signal_wifi_connected_no_internet_4,
                    size: 18),
                label: const Text(
                  'Đang offline',
