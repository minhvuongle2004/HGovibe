...
class Destination {
  ...
  factory Destination.fromFirestore(DocumentSnapshot doc) {
    ...
    return Destination(
      ...
      estimatedCost: data['estimated_cost'] != null
          ...
          : null,
      entranceFee: data['entrance_fee'] != null ? _toInt(data['entrance_fee']) : null,
  ...
  String? get formattedPrice {
    if (estimatedCost != null && estimatedCost!.totalSuggested > 0) {
      return 'Từ ₫ ${estimatedCost!.totalSuggested.toString().replaceAllMapped(
            RegExp(r'(\d)(?=(\d{3})+(?!\d))'),
            (Match m) => '${m[1]},',
          )}';
    } else if (entranceFee != null && entranceFee! > 0) {
      return 'Từ ₫ ${entranceFee.toString().replaceAllMapped(
            RegExp(r'(\d)(?=(\d{3})+(?!\d))'),
            (Match m) => '${m[1]},',
          )}';
    }
