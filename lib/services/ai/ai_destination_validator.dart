...
class AIDestinationValidator {
  ...
  static ValidationResult validateSuggestion(AIActivitySuggestion suggestion) {
    ...
    final coordResult = validateCoordinates(
      suggestion.latitude,
  ...
  static ValidationResult validateCoordinates(double? lat, double? lng) {
    ...
    if (lat < _minLatitude || lat > _maxLatitude) {
      errors.add(
        'Latitude $lat nằm ngoài phạm vi Việt Nam ($_minLatitude - $_maxLatitude)',
      );
    }

    if (lng < _minLongitude || lng > _maxLongitude) {
      errors.add(
        'Longitude $lng nằm ngoài phạm vi Việt Nam ($_minLongitude - $_maxLongitude)',
      );
    }

    // Kiểm tra precision (ít nhất 4 chữ số thập phân) - chỉ warn nếu thực sự thấp
    final latPrecision = lat.toString().split('.').length > 1
        ? lat.toString().split('.')[1].length
        : 0;
    final lngPrecision = lng.toString().split('.').length > 1
        ? lng.toString().split('.')[1].length
        : 0;

    if (latPrecision < 3) {
      // Chỉ warn nếu < 3 chữ số (quá thấp)
      warnings.add(
        'Latitude precision thấp ($latPrecision chữ số, nên có ít nhất 4 chữ số thập phân)',
      );
    }
    ...
    if (lngPrecision < 3) {
      // Chỉ warn nếu < 3 chữ số (quá thấp)
      warnings.add(
        'Longitude precision thấp ($lngPrecision chữ số, nên có ít nhất 4 chữ số thập phân)',
      );
    }
  ...
  static ValidationResult validateName(String name) {
    ...
    if (RegExp(
      r'^(restaurant|cafe|shop|place)\s*\d*$',
      caseSensitive: false,
    ).hasMatch(name)) {
  ...
  static ValidationResult validateCategory(String? category) {
    ...
    if (!_allowedCategories.contains(category.toLowerCase())) {
      errors.add(
        'Category "$category" không được hỗ trợ. Allowed: ${_allowedCategories.join(', ')}',
      );
    }
    ...
    for (final existing in existingDestinations) {
      ...
      if (existingLat != null && existingLng != null) {
        ...
        if (distance < duplicateThresholdKm) {
          errors.add(
            'Địa điểm trùng lặp với "${existing['name']}" (cách ${(distance * 1000).toInt()}m)',
          );
        } else if (distance < 0.5) {
          // 500m warning
          warnings.add(
            'Gần địa điểm "${existing['name']}" (cách ${(distance * 1000).toInt()}m)',
          );
        }
      }
    }
