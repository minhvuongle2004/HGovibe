...
class DestinationProvider extends ChangeNotifier {
  ...
  Future<void> loadDestinationsByCity(String city, {int limit = 20}) async {
    ...
    try {
      _destinations = await DestinationService.getDestinationsByCity(
        city,
        limit: limit,
      );
      _error = null;
  ...
  Future<void> loadDestinationsByRegion(String region, {int limit = 20}) async {
    ...
    try {
      _destinations = await DestinationService.getDestinationsByRegion(
        region,
        limit: limit,
      );
      _error = null;
  ...
  Future<void> loadDestinationsByTags(
    List<String> tags, {
    int limit = 20,
  }) async {
    _isLoading = true;
    ...
    try {
      _destinations = await DestinationService.getDestinationsByTags(
        tags,
        limit: limit,
      );
      _error = null;
  ...
  Future<void> loadDestinationsByMonth(int month, {int limit = 20}) async {
    ...
    try {
      _destinations = await DestinationService.getDestinationsByMonth(
        month,
        limit: limit,
      );
      _error = null;
  ...
  Future<void> loadSeasonalDestinations({int limit = 10}) async {
    ...
    try {
      ...
      _seasonalDestinations = await DestinationService.getDestinationsByMonth(
        currentMonth,
        limit: limit,
      );
    } catch (e) {
  ...
  Future<void> loadDestinationsByTag(String tag, {int limit = 10}) async {
    ...
    try {
      _tagDestinations = await DestinationService.getDestinationsByTags([
        tag,
      ], limit: limit);
    } catch (e) {
