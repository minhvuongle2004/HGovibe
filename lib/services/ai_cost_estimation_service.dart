...
class AICostEstimationService {
  ...
  Future<TripCostEstimate?> estimateTripCost(Trip trip, {double multiplier = 1.0}) async {
    try {
      // Build prompt
      ...
      if (response != null) {
        ...
        if (estimate != null) {
          debugPrint('✅ AI cost estimation completed (multiplier: $multiplier)');
          return estimate;
        }
        ...
        descMatch = RegExp(r'\[DETAILED_DESCRIPTION\]([\s\S]*?)\[/DETAILED_DESCRIPTION\]', caseSensitive: false).firstMatch(response);
        
        // Pattern 2: Nếu không có closing tag, lấy từ [DETAILED_DESCRIPTION] đến cuối hoặc đến [JSON]
        if (descMatch == null) {
          descMatch = RegExp(r'\[DETAILED_DESCRIPTION\]([\s\S]*?)(?:\[/DETAILED_DESCRIPTION\]|\[JSON\]|$)', caseSensitive: false).firstMatch(response);
        }
        
        // Pattern 3: Nếu vẫn không có, tìm từ "DETAILED_DESCRIPTION" (không có brackets)
        if (descMatch == null) {
          descMatch = RegExp(r'DETAILED_DESCRIPTION[:\s]*([\s\S]*?)(?:\[/DETAILED_DESCRIPTION\]|\[JSON\]|$)', caseSensitive: false).firstMatch(response);
        }
        
        if (descMatch != null) {
          aiDescription = descMatch.group(1)?.trim();
          if (aiDescription != null && aiDescription.isNotEmpty) {
            debugPrint('💡 AI trả về description (${aiDescription.length} chars) nhưng không có JSON. Sẽ dùng fallback với description từ AI.');
          } else {
  ...
  String _buildPrompt(Trip trip, double multiplier) {
    ...
    buffer.writeln('Bạn là chuyên gia du lịch. Hãy ước tính chi phí và trả về KẾT QUẢ DƯỚI DẠNG JSON.');
    buffer.writeln('');
    buffer.writeln('**YÊU CẦU: Trả về JSON trước, sau đó mới mô tả chi tiết.**');
    buffer.writeln('');
    ...
    buffer.writeln('```');
    buffer.writeln('');
    buffer.writeln('**Lưu ý:** Tất cả giá trị là số nguyên (VND), không có dấu phẩy, không có đơn vị.');
    buffer.writeln('');
    buffer.writeln('---');
    buffer.writeln('');
    ...
    buffer.writeln('- Ngày: ${dateFormat.format(trip.startDate)} - ${dateFormat.format(trip.endDate)}');
    ...
    for (var date in sortedDates) {
      ...
      buffer.writeln('- ${dateFormat.format(date)}: ${items.map((i) => i.destination!.name).join(', ')}');
      for (var item in items) {
        if (item.destination!.entranceFee != null && item.destination!.entranceFee! > 0) {
          buffer.writeln('  • Vé ${item.destination!.name}: ${item.destination!.entranceFee!.toStringAsFixed(0)} VND/người');
        }
      }
    }
    buffer.writeln('');
    ...
    // Di chuyển (rút gọn)
    if (trip.startingLocation != null && trip.startingLocation!.isNotEmpty) {
      final sortedItems = trip.items.where((item) => item.destination != null && item.plannedDate != null).toList()
        ..sort((a, b) => a.plannedDate!.compareTo(b.plannedDate!));
      if (sortedItems.isNotEmpty) {
        buffer.writeln('**Di chuyển:**');
        buffer.writeln('- ${trip.startingLocation} → ${sortedItems.first.destination!.name}');
        for (var i = 0; i < sortedItems.length - 1; i++) {
          buffer.writeln('- ${sortedItems[i].destination!.name} → ${sortedItems[i + 1].destination!.name}');
        }
        buffer.writeln('');
      }
    }
    ...
    if (multiplier != 1.0) {
      buffer.writeln('- Điều chỉnh: ${(multiplier * 100).toStringAsFixed(0)}% ${multiplier > 1.0 ? "cao hơn" : "thấp hơn"}');
    }
    ...
    buffer.writeln('- JSON là bắt buộc, phải có đầy đủ 8 trường. Giá trị là số nguyên (VND).');
    buffer.writeln('- Mô tả chi tiết cũng BẮT BUỘC, phải liệt kê từng ngày với hoạt động và chi phí cụ thể.');
  ...
  Future<List<String>> getAvailableModels() async {
    ...
    for (final baseUrl in baseUrls) {
      try {
        final url = Uri.parse(
          '$baseUrl/models?key=${ApiConfig.geminiApiKey}',
        );
        ...
        if (response.statusCode == 200) {
          ...
          if (models != null) {
            ...
            if (availableModels.isNotEmpty) {
              debugPrint('✅ Found ${availableModels.length} available models from $baseUrl');
        ...
        try {
          ...
          final requestBody = {
            ...
            'generationConfig': {
              ...
              'topK': 40,
              'topP': 0.95,
              'maxOutputTokens': 8192, // Tăng lên 8192 để không bị giới hạn description (Gemini 2.5-pro hỗ trợ tối đa 8192 tokens)
            }
          };
          ...
          if (response.statusCode == 200) {
            ...
            try {
              ...
              debugPrint('⚠️ Model $model with $baseUrl: ${response.statusCode} - $errorMsg');
            } catch (_) {
              debugPrint('⚠️ Model $model with $baseUrl: ${response.statusCode} - $errorBody');
            }
  ...
  TripCostEstimate? _parseAIResponse(String response, Trip trip) {
    try {
      ...
      // Pattern 1: ```json ... ``` hoặc ``` ... ```
      final codeBlockMatch = RegExp(r'```(?:json)?\s*(\{[\s\S]*?\})\s*```', caseSensitive: false, dotAll: true).firstMatch(response);
      if (codeBlockMatch != null) {
        jsonString = codeBlockMatch.group(1)?.trim();
        debugPrint('✅ Found JSON in markdown code block');
      } else {
        // Pattern 2: [JSON]...[/JSON] tags
        final jsonTagMatch = RegExp(r'\[JSON\]([\s\S]*?)\[/JSON\]', caseSensitive: false, dotAll: true).firstMatch(response);
        if (jsonTagMatch != null) {
          ...
          final jsonInTextMatch = RegExp(r'\{[\s\S]*?"entranceFees"[\s\S]*?"perPerson"[\s\S]*?\}', caseSensitive: false, dotAll: true).firstMatch(response);
      ...
      // Pattern 1: [DETAILED_DESCRIPTION]...[/DETAILED_DESCRIPTION]
      RegExpMatch? descMatch = RegExp(r'\[DETAILED_DESCRIPTION\]([\s\S]*?)\[/DETAILED_DESCRIPTION\]', caseSensitive: false, dotAll: true).firstMatch(response);
      if (descMatch != null) {
        detailedDescription = descMatch.group(1)?.trim();
        debugPrint('✅ Found detailed description in tags (${detailedDescription?.length ?? 0} chars)');
        if (detailedDescription != null && detailedDescription.isNotEmpty) {
          final previewLength = detailedDescription.length > 300 ? 300 : detailedDescription.length;
          debugPrint('📝 Description preview (first $previewLength chars): ${detailedDescription.substring(0, previewLength)}...');
        }
      } else {
        // Pattern 2: Tìm trong markdown code block (không phải json)
        // Tìm ``` ... ``` nhưng không phải ```json
        final descCodeBlockMatch = RegExp(r'```(?!json\b)([\s\S]*?)```', caseSensitive: false, dotAll: true).firstMatch(response);
        if (descCodeBlockMatch != null) {
          detailedDescription = descCodeBlockMatch.group(1)?.trim();
          debugPrint('✅ Found detailed description in markdown code block (${detailedDescription?.length ?? 0} chars)');
        } else {
          // Pattern 3: Tìm text sau JSON (vì prompt yêu cầu JSON trước, description sau)
          if (jsonString != null) {
            final jsonEndIndex = response.indexOf(jsonString) + jsonString.length;
            if (jsonEndIndex < response.length) {
              ...
              if (textAfterJson.isNotEmpty && textAfterJson.length > 50) {
                ...
                final cleanedText = textAfterJson
                    .replaceFirst(RegExp(r'^```\s*', caseSensitive: false, multiLine: true), '')
                    .replaceFirst(RegExp(r'\s*```$', caseSensitive: false, multiLine: true), '')
                    .trim();
                if (cleanedText.isNotEmpty && cleanedText.length > 50) {
                  detailedDescription = cleanedText;
                  debugPrint('✅ Found detailed description after JSON (${detailedDescription.length} chars)');
                  debugPrint('📝 Description preview (first 200 chars): ${detailedDescription.substring(0, detailedDescription.length > 200 ? 200 : detailedDescription.length)}...');
                }
              }
            }
          }
      ...
      if (jsonString != null) {
        ...
        jsonString = jsonString.replaceFirst(RegExp(r'^```(?:json)?\s*', caseSensitive: false, multiLine: true), '');
        jsonString = jsonString.replaceFirst(RegExp(r'\s*```$', caseSensitive: false, multiLine: true), '');
      ...
      if (jsonString == null || jsonString.isEmpty) {
        ...
        debugPrint('Response preview (first 500 chars): ${response.substring(0, response.length > 500 ? 500 : response.length)}...');
        return null;
      }
      ...
      final estimate = TripCostEstimate(
        ...
        aiModel: _lastUsedModel ?? 'gemini-2.5-pro', // Model đã được sử dụng thành công
        detailedDescription: detailedDescription,
      );
      
      // Log để debug
      if (detailedDescription != null) {
        debugPrint('📊 Final estimate - description length: ${detailedDescription.length} chars');
        debugPrint('📝 Description preview (first 300 chars): ${detailedDescription.substring(0, detailedDescription.length > 300 ? 300 : detailedDescription.length)}...');
      } else {
      ...
      debugPrint('Response preview: ${response.substring(0, response.length > 500 ? 500 : response.length)}...');
      debugPrint('$stack');
      return null;
    }
  }
  ...
  TripCostEstimate _fallbackEstimate(Trip trip, double multiplier) {
    ...
    var accommodation = accommodationPerRoomPerNight * numberOfRooms * (trip.daysCount - 1);
    ...
    final subtotal = entranceFees + food + accommodation + transportation + shopping;
  ...
  String _generateFallbackDescription(Trip trip, double multiplier) {
    ...
    final foodPerPersonPerDay = 150000 * (trip.budgetLevel == TripBudgetLevel.budget ? 1 : trip.budgetLevel == TripBudgetLevel.moderate ? 2 : 4) * multiplier;
    final accommodationPerRoomPerNight = 300000 * (trip.budgetLevel == TripBudgetLevel.budget ? 1 : trip.budgetLevel == TripBudgetLevel.moderate ? 2 : 5) * multiplier;
    ...
    for (var dayIndex = 0; dayIndex < trip.daysCount; dayIndex++) {
      ...
      buffer.writeln('## Ngày ${dayIndex + 1} (${dateFormat.format(currentDate)})');
      buffer.writeln('');
      ...
      if (itemsForDay.isNotEmpty && itemsForDay.first.destination != null) {
        ...
        // Tìm điểm trước đó
        String? fromLocation;
        if (dayIndex == 0 && trip.startingLocation != null && trip.startingLocation!.isNotEmpty) {
          // Ngày đầu: di chuyển từ điểm xuất phát
          fromLocation = trip.startingLocation;
          transportationCostForDay = transportationPerPersonPerDay * trip.numberOfTravelers * 0.5; // Chi phí di chuyển đường dài
          transportationNote = 'Di chuyển từ $fromLocation đến ${currentDest.name} (${currentDest.location.city}): Taxi/Xe khách - ~${transportationCostForDay.toStringAsFixed(0)} VND';
        } else {
          ...
          for (var i = 0; i < sortedItems.length; i++) {
            if (sortedItems[i].destination?.id == currentDest.id) {
              ...
              if (i > 0) {
                ...
                final isSameCity = prevDest.location.city == currentDest.location.city;
                transportationCostForDay = isSameCity 
                    ? transportationPerPersonPerDay * trip.numberOfTravelers * 0.2 // Di chuyển trong thành phố
                    : transportationPerPersonPerDay * trip.numberOfTravelers * 0.5; // Di chuyển đường dài
                transportationNote = 'Di chuyển từ $fromLocation (${prevDest.location.city}) đến ${currentDest.name} (${currentDest.location.city}): ${isSameCity ? "Taxi/Xe máy" : "Xe khách/Máy bay"} - ~${transportationCostForDay.toStringAsFixed(0)} VND';
              }
              break;
            }
          }
      ...
      if (itemsForDay.isNotEmpty) {
        for (var item in itemsForDay) {
          if (item.destination != null) {
            ...
            if (entranceFee > 0) {
              buffer.writeln('- Tham quan ${item.destination!.name}: Vé vào cửa ~${(entranceFee * trip.numberOfTravelers).toStringAsFixed(0)} VND');
            } else {
        ...
        buffer.writeln('- Nghỉ ngơi/Tham quan tự do (không có điểm đến cố định)');
      }
      buffer.writeln('- Ăn sáng: ~${(foodPerPersonPerDay * trip.numberOfTravelers * 0.2).toStringAsFixed(0)} VND');
      buffer.writeln('');
      ...
      buffer.writeln('- Ăn trưa: ~${(foodPerPersonPerDay * trip.numberOfTravelers * 0.4).toStringAsFixed(0)} VND');
      buffer.writeln('');
      ...
      buffer.writeln('- Mua sắm/Quà lưu niệm: ~${(shoppingPerPerson * trip.numberOfTravelers / trip.daysCount).toStringAsFixed(0)} VND');
      buffer.writeln('');
      
      // Tối
      buffer.writeln('**Tối:**');
      buffer.writeln('- Ăn tối: ~${(foodPerPersonPerDay * trip.numberOfTravelers * 0.4).toStringAsFixed(0)} VND');
      if (dayIndex < trip.daysCount - 1) {
        final numberOfRooms = (trip.numberOfTravelers / 2).ceil();
        buffer.writeln('- Nghỉ đêm: ~${(accommodationPerRoomPerNight * numberOfRooms).toStringAsFixed(0)} VND');
      }
      
      // Tổng ngày (bao gồm chi phí di chuyển nếu có)
      final entranceFeesForDay = itemsForDay.isNotEmpty 
          ? itemsForDay.map((item) => (item.destination?.entranceFee ?? 0) * trip.numberOfTravelers).fold(0.0, (a, b) => a + b)
          : 0.0;
      final dayTotal = (foodPerPersonPerDay * trip.numberOfTravelers) +
          (dayIndex < trip.daysCount - 1 ? accommodationPerRoomPerNight * (trip.numberOfTravelers / 2).ceil() : 0) +
          transportationCostForDay + // Chi phí di chuyển cụ thể cho ngày này
          (shoppingPerPerson * trip.numberOfTravelers / trip.daysCount) +
          entranceFeesForDay;
      buffer.writeln('');
      buffer.writeln('**Tổng chi phí ngày ${dayIndex + 1}:** ~${dayTotal.toStringAsFixed(0)} VND');
      buffer.writeln('');
    }
    ...
    buffer.writeln('- Ước tính: ~${foodPerPersonPerDay.toStringAsFixed(0)} VND/người/ngày');
    buffer.writeln('');
    buffer.writeln('### Nơi ở');
    buffer.writeln('- Loại: ${trip.budgetLevel.displayName}');
    buffer.writeln('- Ước tính: ~${accommodationPerRoomPerNight.toStringAsFixed(0)} VND/phòng/đêm');
    buffer.writeln('');
    buffer.writeln('### Di chuyển');
    buffer.writeln('- Ước tính: ~${transportationPerPersonPerDay.toStringAsFixed(0)} VND/người/ngày');
    buffer.writeln('');
    buffer.writeln('### Mua sắm');
    buffer.writeln('- Ước tính: ~${shoppingPerPerson.toStringAsFixed(0)} VND/người');
    if (multiplier != 1.0) {
      buffer.writeln('');
      buffer.writeln('*Đã điều chỉnh ${multiplier > 1.0 ? "cao hơn" : "thấp hơn"} ${((multiplier - 1.0).abs() * 100).toStringAsFixed(0)}%*');
    }
    buffer.writeln('');
    buffer.writeln('*Lưu ý: Đây là ước tính dựa trên công thức chuẩn. Để có kế hoạch chi tiết hơn với các cửa hàng cụ thể, vui lòng thử lại tính toán bằng AI.*');
